  ...
  async function handleStart(type) {
    ...
    try {
      ...
      const [introData, qs] = await Promise.all([
        generateIntro(type, user.level, settingsApiKey).catch(() => null),
        isThisDaily
          ? generateDailyChallenge(user.level, settingsApiKey).then(q => [q])
          : generateQuiz(type, SESSION_SIZE, user.level, settingsApiKey),
      ]);
